# Feature-Parity Campaign - Ledger

Backlog from the parity audit (workflow `parity-audit`, 13 agents, 94 findings).
...
- [ ] **[document-mgmt/L]** Detect external on-disk changes to the open file
  - Reference: When the file you have open is modified or deleted by another program (git checkout, another editor, cloud sync), the reference editor notices and prompts to reload (or warns the file was removed), preventing you from silently overwriting external edits on the next save.
...
- [ ] **[block-elements/M]** Add a new row when Tab is pressed in the last table cell
  - Reference: Pressing Tab while in the bottom-right cell of a table appends a new empty row and moves the cursor into its first cell, so you can build a table entirely from the keyboard.
...
- [ ] **[sidebar-filetree/M]** Persist folder expand/collapse state across tree refreshes
  - Reference: Folders stay expanded after you create/rename/delete a file or otherwise refresh the tree; the expansion state is a property of the tree, not of the render.
...
- [ ] **[sidebar-filetree/M]** Auto-reveal and scroll the active file into view
  - Reference: Opening a file (from Open Recent, search, or links) auto-expands its parent folders in the file tree and scrolls the highlighted row into view, so the current document is always locatable in the sidebar.
...
- [ ] **[outline/M]** Highlight the active heading as you scroll the document
  - Reference: the reference editor's outline continuously highlights the heading whose section the viewport is currently in. As you scroll the editor, the highlight moves down the outline automatically, giving a live 'you are here' indicator.
...
- [ ] **[images/M]** Resolve relative assets/ paths to file:// for display
  - Reference: After copying a pasted/dropped image into the document's assets/ folder, the reference editor writes a relative path (e.g. assets/foo.png) into the Markdown AND immediately displays the image inline, because it resolves the relative path against the document's own directory on disk.
...
- [ ] **[math-diagrams/M]** Auto-enter edit mode when a math/diagram node is freshly inserted
  - Reference: When you create a block equation (type `$$` then Enter) or insert inline math, the reference editor drops you straight into the LaTeX source editor with the caret ready, so you can type the formula immediately. Same for a new mermaid block created via the menu.
...
- [ ] **[export-print/M]** Add a real Print command (system print dialog)
  - Reference: the reference editor has File > Print (Cmd/Ctrl+P) that opens the OS print dialog, letting the user pick any installed printer, paper size, page range, copies, or 'Save as PDF' via the native dialog. PDF export is a separate, dedicated command.
...
- [ ] **[themes/M]** Add a "Follow System Appearance" auto theme that switches light/dark with the OS
  - Reference: the reference editor has a built-in "Auto" appearance that follows the OS light/dark setting and live-switches between a chosen light theme and dark theme as macOS/Windows toggles appearance, with no app restart.
...
- [ ] **[themes/M]** Syntax-highlight the source-mode (CodeMirror) markdown instead of rendering it monochrome
  - Reference: the reference editor's source mode colors markdown syntax: headings, bold/italic markers, link/image syntax, inline/fenced code, blockquote markers, list bullets, etc. are visually distinct, matching the active theme.
...
- [ ] **[document-mgmt/M]** Restore the last open document on launch
  - Reference: the reference editor reopens the document(s) you had open when you last quit. Relaunching brings you straight back to your file with the cursor where you left it, so the app feels continuous across restarts.
...
- [ ] **[document-mgmt/M]** Handle macOS Open-With and double-click file open
  - Reference: Double-clicking a .md file in Finder, or using Open With > the reference editor, opens that file in the reference editor. On macOS this is delivered via the app 'open-file' event; the reference editor opens the file in a window.
...
- [ ] **[inline-editing/S]** Add undoInputRule on Backspace to revert auto-formatting
  - Reference: After an auto-format fires (e.g. typing `# ` makes a heading, `- ` makes a bullet, `1. ` makes an ordered list, `> ` a blockquote), pressing Backspace immediately reverts the transform back to the literal typed characters instead of deleting content. This lets users escape an unwanted markdown trigger.
...
- [ ] **[inline-editing/S]** Make inline code mark non-inclusive and add caret-exit for trailing marks
  - Reference: After you type a closing inline-code delimiter (or bold/italic), the caret sits OUTSIDE the mark: typing more text is plain, not code/bold. Pressing the Right arrow past a closing delimiter likewise drops you out of the mark. You are never 'stuck' typing in code once you closed it.
...
- [ ] **[block-elements/S]** Add a hard line break on Shift-Enter
  - Reference: Pressing Shift+Enter inside a paragraph/heading/list item inserts a soft line break (a trailing-space or backslash break in the markdown, rendered as <br>), letting you wrap text within the same block without starting a new paragraph.
...
- [ ] **[find-replace/S]** Seed the find query from the current selection on open
  - Reference: When you press Cmd/Ctrl+F with text selected (or even the word under the cursor), the reference editor pre-fills the find field with that text and selects it, so you can immediately hit Enter to jump to the next occurrence.
...
- [ ] **[find-replace/S]** Scroll the current match into view as soon as the query is typed
  - Reference: As you type in the reference editor's find bar, the first match is highlighted AND the document scrolls to it immediately - you don't have to press Enter to see where the match is.
...
- [ ] **[images/S]** Constrain inline image size with max-width
  - Reference: Inline images are clamped to the content column width (max-width:100%) so a large photo never overflows the editor or pushes layout; images scale down responsively.
  ...
  - Change: Add a `.ProseMirror img { max-width: 100%; height: auto; }` rule (plus a sensible vertical rhythm / cursor) to global.css so inline images scale to the content width like a polished editor.
  - Files: `global.css`
- [ ] **[math-diagrams/S]** Add the `$$` keyboard gesture to create a block equation
  - Reference: Typing `$$` and pressing Enter (or space) on an empty line converts the line into a centered block-equation editor - the canonical, discoverable way to start display math without menus.
...
- [ ] **[export-print/S]** Copy as HTML should produce an inline fragment, not a full standalone document
  - Reference: the reference editor's Edit > Copy as HTML (and Copy as HTML Code) writes a clean HTML body fragment to the clipboard so pasting into Word, Gmail, Notion, etc. yields properly styled inline content without a leaked <head>/<style>/<!DOCTYPE> block.
...
- [ ] **[shortcuts-palette/S]** Match the reference editor's heading shortcuts (Cmd+1..6 / Cmd+0), not Cmd+Alt+1..6
  - Reference: Headings are set with Cmd+1 through Cmd+6 (Ctrl+1..6 on Windows/Linux), and paragraph with Cmd+0. These are the single most-used formatting shortcuts and are muscle-memory for such users.
...
- [ ] **[block-elements/L]** Auto-format a typed GFM pipe-table row into a real table
  - Reference: Typing a header row plus the separator line (e.g. `| a | b |` then `|---|---|`) auto-converts the text into a live editable table. the reference editor also offers menu/shortcut insert with a size picker.
...
- [ ] **[sidebar-filetree/L]** Support drag-and-drop to move files/folders within the tree
  - Reference: You can drag a file or folder onto another folder in the file tree to move it on disk; the tree updates in place.
...
- [ ] **[find-replace/L]** Add whole-word and regex toggles
  - Reference: the reference editor's find bar has three toggles: case sensitivity (Aa), whole word (W / \b), and regex (.*). Regex find/replace (including capture-group substitution like $1) is a commonly used the reference editor feature.
...
- [ ] **[images/L]** Support per-image width control (resize handles / width attribute)
  - Reference: the reference editor lets you set an image's display width - either by dragging resize handles or via the image context menu (e.g. 'Set Image Width' 25/50/100%) - and persists it by emitting an HTML width attribute or `{width=...}` so the size round-trips in the Markdown.
...
- [ ] **[document-mgmt/L]** Autosave untitled documents to recover unsaved work
  - Reference: the reference editor keeps unsaved/untitled buffers backed by an on-disk draft, so an unexpected quit or crash does not lose typing in a never-saved document; on relaunch the draft is restored.
...
- [ ] **[inline-editing/M]** Auto-pair markdown delimiters around a selection
  - Reference: With text selected, typing `*`, `_`, `` ` ``, `(`, `[`, `{`, `"`, or `'` wraps the selection in that pair (e.g. select a word, press `*`, get `*word*`) rather than replacing it. This is the fastest way to emphasize existing text by keyboard.
...
- [ ] **[inline-editing/M]** Auto-linkify a URL typed inline (not just pasted onto a selection)
  - Reference: Typing a bare URL (e.g. `https://example.com`) followed by a space or Enter automatically converts it into a clickable link. Selection-paste of a URL also links, but plain typed URLs are auto-detected too.
...
- [ ] **[inline-editing/M]** Add a paste-as-plain-text shortcut (Mod-Shift-V)
  - Reference: Cmd/Ctrl+Shift+V pastes the clipboard as unformatted plain text, stripping any HTML/markdown structure - the standard 'paste and match style' gesture.
...
- [ ] **[inline-editing/M]** Bind Mod-K to insert/edit a link
  - Reference: Cmd/Ctrl+K opens the link insertion flow: with text selected it wraps the selection in a link, otherwise it lets you type the URL. It is the standard keyboard path to linking.
...
- [ ] **[block-elements/M]** Exit a code block by pressing Enter twice on a trailing blank line
  - Reference: In a code block, pressing Enter on an empty last line (or Enter twice at the end) drops you out of the code block into a new paragraph below. Mod-Enter also exits.
...
- [ ] **[block-elements/M]** Let Tab/Shift-Tab nest task-list items and keep checkbox state
  - Reference: Inside a task list, Tab indents the item into a nested sub-list (and Shift-Tab outdents), preserving its checkbox.
...
- [ ] **[typography/M]** Match the reference editor's loose-list paragraph spacing
  - Reference: In the reference editor, list ITEMS that contain block content (loose lists, or items with multiple paragraphs) keep vertical paragraph spacing, and there is a small gap (~4-6px) between sibling list items. Single-line items are tight, but multi-paragraph items breathe.
  - Lekha: github.css collapses ALL li > p margins to 0 unconditionally (lines 509-511, 'margin: 0'), and li margin is only 0.15em (line 504). This makes every list - including loose lists with multi-paragraph items - render as tightly packed lines with no separation between paragraphs inside an item or between loose items. A user pasting a loose list will see it visually flattened versus the reference.
...
- [ ] **[sidebar-filetree/M]** Make folder-search results jump to the exact matched line, and clickable per-match
  - Reference: Clicking a specific search result opens the file and navigates to that exact occurrence/line; multiple matches in a file are individually navigable.
...
- [ ] **[find-replace/M]** Add Find Next / Find Previous accelerators (Cmd+G / F3) usable when the bar is closed
  - Reference: After a search, the reference editor lets you repeat it with the bar closed (F3 / Shift+F3 on Windows, Cmd+G / Cmd+Shift+G on macOS), cycling matches without the find UI grabbing focus.
...
- [ ] **[images/M]** Expose title field and edit-existing-image flow in the Image dialog
  - Reference: Selecting an existing image and re-opening the image dialog lets you edit src, alt and the image title/tooltip; the title round-trips as `![alt](src "title")`.
...
- [ ] **[math-diagrams/M]** Surface KaTeX parse errors as readable messages, not raw red LaTeX
  - Reference: On invalid LaTeX, the reference editor shows the source in red with KaTeX's specific error message (e.g. "Undefined control sequence: \foo") so you know what to fix.
...
- [ ] **[math-diagrams/M]** Provide a visible click-to-edit affordance and keyboard entry into math source
  - Reference: Hovering/clicking rendered math gives a clear cue it is editable; selecting it and pressing Enter opens the source.
...
- [ ] **[export-print/M]** Export should honor the active editor theme instead of hardcoding GitHub
  - Reference: the reference editor exports/prints using the currently selected theme's CSS (Night, GitHub, Newsprint, etc.), so the exported HTML/PDF visually matches what the user sees in the editor. Themes can also be picked per-export.
...
- [ ] **[export-print/M]** Expose PDF page options (size, margins, orientation)
  - Reference: the reference editor's PDF export (via the print dialog / export settings) lets the user choose paper size (A4/Letter/etc.), orientation, and margins, and remembers preferences.
...
- [ ] **[shortcuts-palette/M]** Expose insert-table / math-block / diagram / footnote as commands in the palette and menu
  - Reference: the reference editor's Paragraph and Format menus include Table (Cmd+Option+T), Math Block (Cmd+Option+B), and footnote insertion, and these are reachable via menu/shortcut, not only by typing markdown.
...
- [ ] **[shortcuts-palette/M]** Add a keyboard shortcut to exit a code block / fenced block (Escape or Cmd+Enter)
  - Reference: From inside a fenced code block, pressing the down arrow at the last line or Cmd+Enter/Escape moves the caret out into a new paragraph below, so the user is never trapped. the reference editor also lets a trailing Enter on an empty last line break out.
...
- [ ] **[shortcuts-palette/M]** Add commands/shortcuts for highlight, superscript, subscript marks
  - Reference: the reference editor supports highlight (==text==), superscript (^text^), and subscript (~text~) and exposes them in the Format menu; highlight also has a configurable shortcut in many builds.
...
- [ ] **[micro-interactions/M]** Surface Focus/Typewriter mode state in the status bar
  - Reference: the reference editor reflects active editing modes in its chrome - Focus Mode and Typewriter Mode are toggles whose on/off state is discoverable (menu checkmarks plus the visible dimming), and users can flip them without remembering a function key.
...
- [ ] **[micro-interactions/M]** Differentiate the dirty indicator from the auto-save state
  - Reference: the reference editor's title bar shows an unsaved-changes marker, and because edits are written to disk it does not leave files perpetually 'dirty'; the chrome accurately reflects whether on-disk state matches the buffer.
  - Lekha: TitleBar.tsx shows a bullet '•' whenever isDirty is true. App.tsx's handleChange sets isDirty on every keystroke, while useAutoSave silently writes after idle. Because there is no UI feedback when the auto-save fires, the dot can linger or flicker and the user gets no 'Saving…/Saved' confirmation - the auto-save (a defining behavior) is completely invisible.
...
- [ ] **[block-elements/S]** Indent code-block lines with Tab instead of leaving the block
  - Reference: Inside a fenced code block, Tab inserts indentation (spaces/tab) into the code rather than moving focus, matching a code editor.
  ...
  - Files: `keymap.ts`
- [ ] **[typography/S]** Make headings bold (700) to match the reference editor's weight
  - Reference: In the reference editor's default GitHub theme every heading h1-h6 uses font-weight: bold (700). h1 is ~2em and h2 ~1.5em, both heavy. The headings read as clearly heavier than body text, which is the dominant visual cue of the reference editor's hierarchy.
  - Lekha: github.css sets the shared heading rule to font-weight: 600, then explicitly OVERRIDES h1 and h2 down to font-weight: 500 (lines 426-438). So Lekha's two most prominent headings are semibold-to-medium, noticeably lighter and airier than the reference editor. The comment even calls this an intentional 'elegant, airy title' deviation.
...
- [ ] **[sidebar-filetree/S]** Show the open folder name as the Files panel header
  - Reference: The file-tree panel header shows the name of the currently open root folder (the workspace), so you know which folder you are browsing.
...
- [ ] **[sidebar-filetree/S]** Add a Collapse All / expand-all control to the file tree
  - Reference: The file tree offers a collapse-all action (and folders can be quickly folded), useful for large workspaces.
...
- [ ] **[outline/S]** Auto-scroll the outline panel to keep the active heading in view
  - Reference: When the active heading moves outside the visible region of a long outline, the reference editor scrolls the outline panel so the current heading stays visible.
...
- [ ] **[outline/S]** Scroll clicked heading to the top instead of centering, and don't move the cursor/steal focus
  - Reference: Clicking an outline entry scrolls that heading to (near) the top of the editor so its whole section is visible below. The click is a navigation gesture; it does not place the text caret mid-document or change the selection.
...
- [ ] **[outline/S]** Reset collapsed outline state when the active document changes
  - Reference: Each document's outline collapse state is per-document; opening a different file shows that file's outline freshly (not collapsed by stale positions from the previous file).
...
- [ ] **[math-diagrams/S]** Inline math: live preview while editing (currently only block math previews)
  - Reference: While editing inline math, the reference editor renders the formula live as you type, right where it sits.
...
- [ ] **[math-diagrams/S]** Debounce KaTeX re-render on keystroke (mermaid is debounced, math is not)
  - Reference: the reference editor re-renders math smoothly while typing without visible flicker on each keystroke.
...
- [ ] **[export-print/S]** Add a print/PDF stylesheet for page breaks and clean pagination
  - Reference: the reference editor supports CSS page-break control in exports (e.g. headings not orphaned, code blocks and tables avoiding awkward splits, and the theme's @media print rules) so multi-page PDFs paginate cleanly.
...
- [ ] **[export-print/S]** Surface export failures to the user instead of only console.error
  - Reference: the reference editor shows a visible dialog/notification when an export fails (e.g. Pandoc missing, write error), telling the user what went wrong and how to fix it.
...
- [ ] **[themes/S]** Remove the h1/h2 underline rules on dark/non-GitHub themes
  - Reference: Only the reference GitHub theme draws a hairline border-bottom under h1/h2. Dark themes (Night, Newsprint, Pixyll, etc.) have NO heading underlines - headings are just larger/bolder text.
  - Lekha: github.css unconditionally applies border-bottom: 1px solid var(--border-light); padding-bottom: 0.3em to h1 and h2 (lines 425-438). None of the dark/alt themes (graphite.css, night.css, nord.css, solarized-dark.css) override this, so every theme - including 'Graphite', explicitly described as a faithful calm-dark clone - shows GitHub-style underlined h1/h2, which a calm dark theme should not.
...
- [ ] **[themes/S]** Set a theme-matched window backgroundColor to kill the white launch flash in dark themes
  - Reference: the reference editor paints its window background to the theme color, so launching in a dark theme shows a dark window immediately with no white flash.
...
- [ ] **[themes/S]** Sync macOS native chrome (titlebar / traffic lights / context menus) to dark themes
  - Reference: When a dark theme is active, the reference editor's window chrome and native menus render in dark mode on macOS.
...
- [ ] **[themes/S]** Theme the source-mode caret, active-line, and gutter selection; widen ::selection beyond .ProseMirror
  - Reference: the reference editor's editor selection, active line, and caret all respect the theme in both rendered and source views.
...
- [ ] **[shortcuts-palette/S]** Add indent / outdent shortcuts (Cmd+] / Cmd+[)
  - Reference: Cmd+] indents (sinks a list item or adds blockquote/indent nesting) and Cmd+[ outdents, working anywhere in the document, not only via Tab inside a list.
  - Lekha: keymap.ts only handles Tab / Shift-Tab, and those are chained through goToNextCell then sinkListItem/liftListItem (lines 44-53, 69-70). There is no Mod-] / Mod-[ binding at all, so users who press Cmd+] (the reference editor muscle memory) get nothing, and indenting requires Tab which also competes with focus traversal expectations.
...
- [ ] **[shortcuts-palette/S]** Give block-formatting commands keyboard shortcuts (quote, code block, lists)
  - Reference: the reference editor binds the common block conversions: Cmd+Shift+K = code block, Cmd+Shift+Q = blockquote, Cmd+Shift+] = ordered list, Cmd+Shift+[ = unordered list (Ctrl+Shift on Windows). These appear next to the items in the reference editor's Paragraph menu.
  ...
  - Change: Add bindings in keymap.ts for the block commands already in cmds: 'Mod-Shift-q': cmds.blockquote, 'Mod-Shift-k': cmds.codeBlock, 'Mod-Shift-]': cmds.orderedList, 'Mod-Shift-[': cmds.bulletList (pick mappings that don't collide with Mod-] indent above; if collision, use the reference editor's exact set). Mirror the accelerators in menu.ts (lines 263-268) and the shortcut hints in registry.ts (lines 65-69).
  - Files: `keymap.ts`, `menu.ts`, `registry.ts`
- [ ] **[micro-interactions/S]** Add an empty-document placeholder prompt
  - Reference: When a document is empty, the reference editor shows a faint, centered placeholder hint in the editor body (e.g. 'Write something...'/'Type here...') that disappears the moment you type. New/empty files never look like a blank void.
...
- [ ] **[micro-interactions/S]** Label the source-mode toggle by the action, not the current state
  - Reference: the reference editor's source/preview toggle and menu items name the action you will perform ('Source Code Mode' enters source; you switch back via the same labeled control), so the button text never reads as a redundant status echo of where you already are.
...
- [ ] **[document-mgmt/S]** Enforce single-instance and route second-launch file opens
  - Reference: Opening additional files while the reference editor is running reuses the existing process (a new window in the same instance) rather than spawning a separate, state-unaware app instance.
...
- [ ] **[document-mgmt/S]** Prune or flag missing recent files and surface open errors
  - Reference: the reference editor's Open Recent skips/greys files that no longer exist and removes dead entries; clicking a recent file that is gone does not throw an unhandled error.
...
- [ ] **[export-print/L]** Embed images/relative resources in exported HTML so it is truly self-contained
  - Reference: the reference editor's exported HTML can embed local images as base64 data URIs so the single .html file renders anywhere with no missing-image placeholders.
...
- [ ] **[block-elements/M]** Offer a size picker (or grid) when inserting a table
  - Reference: The Table insert flow lets you choose rows x columns (drag-grid or a small dialog) so you get the table you want immediately.
...
- [ ] **[block-elements/M]** Convert an empty heading/quote back to a paragraph on Backspace
  - Reference: Pressing Backspace at the very start of an empty heading or an empty blockquote line converts it back to a plain paragraph (or lifts out of the quote) rather than joining with the previous block.
  - Lekha: Only baseKeymap's generic backspace (joinBackward) runs - there is no setBlockType-to-paragraph or liftEmptyBlock-style handling tuned for headings/blockquotes. Backspace at start of an empty heading joins it into the previous block instead of demoting it to a paragraph, which is the common the reference editor gesture to undo an accidental heading.
...
- [ ] **[sidebar-filetree/M]** Let clicking a folder row's name toggle without forcing arrow-only, and don't lose selection of files inside
  - Reference: Folder rows toggle on click anywhere on the row, and the tree distinguishes the selected/active row from merely-open folders; new entries are created inline at the right place.
...
- [ ] **[images/M]** Preserve original file extension/name and dedupe instead of generic image-NNNNNN
  - Reference: When you drag/drop or paste an image file, the reference editor keeps the original filename in the assets folder where possible, so the asset is recognizable on disk.
...
- [ ] **[images/M]** Handle dropped/pasted image URLs and HTML, not just file bytes
  - Reference: Dragging an image from a browser, or pasting copied HTML/image, inserts the image by its URL (or downloads it), not just raw file bitmaps.
...
- [ ] **[images/M]** Add real zoom/pan controls to the image lightbox
  - Reference: the reference editor's image preview/zoom lets you actually zoom in beyond fit (and the OS preview supports pan), useful for inspecting detail.
...
- [ ] **[micro-interactions/M]** Show reading time (and ideally cursor position) in the status bar
  - Reference: the reference editor's word-count widget in the status bar shows words, characters, lines, and reading time at a glance, and clicking expands the full breakdown. Reading time is visible without opening a panel.
...
- [ ] **[micro-interactions/M]** Make focus mode dim by paragraph/sentence consistently and ensure it works in lists
  - Reference: the reference editor's Focus Mode highlights the current paragraph (and offers sentence-level focus), highlighting the active line even inside list items and blockquotes.
  - Lekha: focusMode.ts decorates only the active TOP-LEVEL block (findActiveTopLevelBlock, direct child of doc) and github.css dims `.ProseMirror > *`. Inside a long list or blockquote the entire list/quote is treated as one block - every item stays at full opacity together rather than focusing the current line, so the focus effect is coarse compared to the reference editor's per-paragraph behavior.
...
- [ ] **[inline-editing/S]** Scope the em-dash rule so `--` does not fire inside words
  - Reference: the reference editor is conservative about converting `--`; in normal prose it generally leaves intra-word hyphenation (e.g. `well--known`) untouched and primarily targets standalone dash sequences, avoiding surprise glyph substitution.
  ...
  - Files: `inputRules.ts`
- [ ] **[typography/S]** Align H1/H2 font sizes with the reference editor's scale
  - Reference: the reference editor default GitHub theme: h1 = 2em, h2 = 1.5em, h3 = 1.25em, h4 = 1em, h5 = 0.875em, h6 = 0.85em (the GitHub markdown scale).
  - Lekha: github.css uses h1 = 2.25em (line 426), h2 = 1.65em (line 434), h3 = 1.35em (line 441), h4 = 1.1em (line 444), h5 = 0.95em (line 448), h6 = 0.88em (line 452). Every heading is a step larger than the reference editor, and h1 also carries letter-spacing: -0.01em which the reference editor does not.
  ...
  - Files: `github.css`
- [ ] **[typography/S]** Lower default body line-height from 1.7 to the reference editor's 1.6
  - Reference: the reference editor default theme sets body line-height to 1.6. Paragraph rhythm in the reference editor is comparatively tight; the 1.6 value is a recognizable part of its look.
  - Lekha: github.css sets --editor-line-height: 1.7 (line 205) for the default theme, and the task-item checkbox height is hard-coded to 1.7em (github.css line 696) to match it. Graphite bumps it further to 1.75 (graphite.css line 28). The default reading column is therefore looser than the reference editor.
  ...
  - Files: `github.css`
- [ ] **[typography/S]** Give inline code a border and tune background like a polished editor
  - Reference: the reference editor default GitHub inline code uses background rgba(0,0,0,0.05)-ish (a faint gray), NO border in the default theme, padding ~2px 4px, font-size ~0.9em, and color matching body. The key recognizable trait is the subtle rounded tint. (Many the reference editor themes also add a hairline border.)
  - Lekha: github.css inline code (lines 539-546) uses --code-bg-inline (#f8f8f8 light), border-radius 3px, padding 0.2em 0.4em, font-size 0.9em, no border. This is close, but the #f8f8f8 tint is extremely faint against the #ffffff page (only ~3% gray) so inline code barely stands out compared with the reference editor's slightly stronger wash. In Night/Graphite the inline bg equals the fenced code bg, which is fine.
  - Change: Darken --gh-code-bg-inline slightly (e.g. to rgba(27,31,35,0.05) or #eff1f3) so inline code reads as a distinct chip on white, matching the reference editor's contrast. Optionally add a 1px solid color-mix border for extra parity. Low priority since the structure is already correct.
  - Files: `github.css`
- [ ] **[typography/S]** Reduce blockquote left-border thickness toward the reference editor's hairline
  - Reference: the reference editor's default GitHub blockquote uses a softer treatment: a 0.25em (~4px is acceptable) gray left bar but with muted text and padding, and importantly the bar color is a light gray (#dfe2e5). It reads quietly. the reference editor actually uses border-left of 4px in GitHub theme but pairs it with a more muted text and a left padding of 1em - Lekha is close here.
  - Lekha: github.css blockquote (lines 474-479): border-left 4px solid --blockquote-border, padding 0 1em, color --text-sub. This is actually a faithful match to the reference editor's GitHub blockquote. No real gap - the bar width, color, padding, and muted text all align.
  - Change: No change needed; Lekha already matches the reference here. (Listed only to confirm parity - safe to skip.)
  - Files: `github.css`
- [ ] **[typography/S]** Use the reference editor's default body font stack instead of Open Sans
  - Reference: the reference editor's default theme body font is a system/serif-agnostic stack led by 'Open Sans' then Helvetica/Arial - so Lekha's --font-editor: "Open Sans", "Helvetica Neue"... is actually faithful to the reference editor default. However the reference dark theme does NOT switch to a different family; it keeps the same body font. Graphite correctly keeps a system sans.
  - Lekha: github.css --font-editor (line 194) = Open Sans / Helvetica Neue / Helvetica / Arial - this matches the reference editor's GitHub theme default. global.css has an unrelated structural fallback of Georgia (global.css line 29) but the theme token wins. No real divergence in the active theme.
  - Change: No change needed for parity; Lekha's default body font already mirrors the reference editor. (Confirmed - safe to skip. Only flag if you want the global.css Georgia fallback aligned to a sans for robustness when no theme loads.)
  - Files: `github.css`, `global.css`
- [ ] **[typography/S]** Tighten heading top/bottom margins to the reference editor's rhythm
  - Reference: the reference editor default GitHub theme heading margins: margin-top: 1em (24px-ish) and margin-bottom: 16px, i.e. roughly 1em top / 1em bottom, fairly even. h1/h2 add padding-bottom: 0.3em above their border.
  - Lekha: github.css shared heading rule (lines 417-419) uses margin-top: 1.6em and margin-bottom: 0.7em. The 1.6em top gap is noticeably larger than the reference editor's ~1em, and the asymmetry (large top, small bottom) is more pronounced than the reference editor's near-even spacing, so sections sit farther apart than in the reference editor.
  - Change: Reduce heading margin-top to ~1em (or 1.2em) and raise margin-bottom toward ~0.6-1em to approximate the reference editor's evener heading rhythm. Adjust lines 418-419 in github.css.
  - Files: `github.css`
- [ ] **[sidebar-filetree/S]** Add 'Copy Path' / 'Copy Relative Path' to the file-tree context menu
  - Reference: The file-tree right-click menu includes Copy Path and Copy Relative Path (alongside reveal, rename, delete, new), commonly used when referencing files in links.
...
- [ ] **[outline/S]** Show a placeholder for empty/untitled headings
  - Reference: Headings with no text still appear as outline entries with a sensible placeholder rather than a blank, unclickable-looking line.
...
- [ ] **[find-replace/S]** Switch between Find and Replace modes without reopening, and let Cmd+Alt+F upgrade an open find bar
  - Reference: In the reference editor the find bar can expand into the replace view in place; invoking replace while find is open just reveals the replace row, keeping the existing query.
...
- [ ] **[find-replace/S]** Report the number of replacements made by Replace All
  - Reference: the reference editor's Replace All gives feedback on how many replacements occurred (and the count drops to 0/0), confirming the operation.
...
- [ ] **[find-replace/S]** Disable Replace/Replace All when there is no match, and keep focus in the find field
  - Reference: the reference editor disables replace actions when there is nothing to replace and keeps the find bar usable throughout.
...
- [ ] **[math-diagrams/S]** Make mermaid render errors readable instead of full raw stack/message dump
  - Reference: the reference editor shows a concise 'Syntax error in graph' style message for invalid mermaid.
...
- [ ] **[export-print/S]** Generate heading id anchors so exported HTML supports in-document links and a TOC
  - Reference: the reference editor assigns slug ids to headings on export so a [[toc]] block and intra-document anchor links (e.g. [link](#section)) work in the exported HTML, and so external deep links to headings resolve.
...
- [ ] **[themes/S]** Differentiate Graphite from Night - it is nearly a duplicate dark theme
  - Reference: the reference editor ships distinct dark themes (Night vs e.g. its other dark options) with genuinely different surfaces, accents, and code palettes; users pick between meaningfully different looks.
  ...
  - Change: Give Graphite its own calm-dark code palette (muted highlight colors distinct from VS Code dark+, e.g. softer keyword/blue/green tones) and a distinct selection (#3f4d5c is already set - good) so it is a real alternative to Night rather than a recolored clone. Adjust --hljs-* and --highlight-bg in graphite.css to a distinct calm-dark set.
  - Files: `graphite.css`
- [ ] **[shortcuts-palette/S]** Bind strikethrough and inline-code to the reference editor's accelerators in the menu
  - Reference: the reference editor binds Strikethrough to Ctrl+Shift+~ (Cmd+Shift+` style) and inline code to Ctrl+Shift+` ; these accelerators show in the Format menu so they are discoverable.
  ...
  - Change: Surface the existing accelerators: in menu.ts set the Strikethrough item accelerator to match the keymap (or align both to the reference editor's Cmd+Shift+`) and give Code an accelerator. Add the matching '⌘⇧X' / '⌘`' (or chosen reference-aligned) shortcut hints to the strikethrough and inlineCode entries in registry.ts so the palette displays them.
...
- [ ] **[micro-interactions/S]** Show the file path on the title (tooltip / full path) instead of bare filename
  - Reference: the reference editor's title bar shows the document name and exposes the full path (hover/tooltip and the proxy-icon path popover on macOS), so users can disambiguate same-named files and see where the file lives.
  ...
  - Files: `TitleBar.tsx`
- [ ] **[document-mgmt/S]** Make autosave debounce/idle behavior closer to the reference editor
  - Reference: the reference editor autosaves on an idle/interval basis and reliably persists before the app quits, so saved files stay current without manual Cmd+S.
...
### Wave 1 - Editor input & keymap parity (`feat/input-parity`)
Implemented:
...
- **heading shortcuts Cmd+1..6 / Cmd+0** (kept Cmd+Alt+0..6 as alternates) - keymap.ts + menu.ts.
...
### Wave 2 - Sidebar file-tree reveal & expansion (`feat/sidebar-reveal`)
Implemented:
...
Note: the active file's own ancestor folders can't be manually collapsed while active (pure-derivation tradeoff to satisfy the react-hooks/refs lint - no effect/ref/setState). Acceptable + familiar.

### Wave 3 - Table & find interaction parity (`feat/table-find`)
Implemented:
...
### Wave 4 - Format menu completeness (`feat/format-menu`)
...
- Updated registry heading shortcuts to Cmd+1..6 / Cmd+0 (matching Wave 1).
...
### Wave 6 - Doc-level file ops, part 1 (`feat/doc-fileops`)
...
### Wave 7 - Doc-level file ops, part 2: Duplicate + Delete (`feat/fileops-2`)
...
### Wave 8 - Move To + Save All (`feat/move-saveall`)
...
### Wave 10 - Rename current document (`feat/rename`)
Implemented:
...
### Wave 12 - Articles/Library sidebar view (`feat/articles`)
...
### Wave 13 - Share via macOS share sheet (`feat/share`)
...
### Wave 14 - Jump to Top / Bottom (`feat/jump`)
...
### Wave 15 - Copy as Plain Text / Copy without Theme Styling (`feat/copy-plain`)
...
### Wave 16 - Paste as Plain Text (`feat/paste-plain`)
...
### Wave 17 - Show Status Bar + Always on Top (`feat/chrome-toggles`)
...
### Wave 18 - Move Row/Column in tables (`feat/table-move`)
...
Table ops stay in the contextual toolbar (not the Edit menu) - matches where the reference editor surfaces them.

Tests: +3 (move row down ordering, move row up validity, move column right ordering). 1230 unit + 7 e2e green.

### Wave 19 - Smart-Punctuation toggle (`feat/smart-punct`)
...
### Wave 20 - Line Endings LF/CRLF (`feat/line-endings`)
...
### Wave 21 - Selection submenu + macOS Speech/Emoji (`feat/selection-speech`)
...
### Wave 22 - Paragraph menu + Indent/Outdent + Heading promote/demote (`feat/paragraph-menu`)
Implemented (h):
- **Dedicated Paragraph menu** (feature parity): Format menu now holds inline formatting + link/image only; block-level structure (Heading 1-6, Paragraph, Increase/Decrease Heading Level, lists, blockquote, code block, HR, Indent/Outdent) moved to a new top-level **Paragraph** menu.
...
### Wave 23a - Document tabs: store foundation (`feat/document-tabs-store`)
...
### Wave 23c - Document tabs: editor/App wiring (`feat/tabs-wiring`)
...
### Wave 23d - Document tabs: persistence + keyboard/menu (`feat/tabs-persistence`)
