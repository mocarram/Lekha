...
export function useStartup(
  ...
  useEffect(() => {
    ...
    void window.lekha.getSettings().then(async (s) => {
      ...
      // has content. Fire-and-forget: the reveal only patches the workspace-tree
      // store, which is disjoint from the crash-recovery / launch-open steps
      // below, so it runs in the background rather than blocking them. Best-
      // effort; failures are swallowed inside revealPath.
      if (ownsSession && s.activeTabPath !== null) {
        void fileOpsRef.current.revealPath(s.activeTabPath)
      }
